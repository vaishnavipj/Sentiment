...
def main():
    ...
    if uploaded:
        ...
        full_text = "\n\n".join([f"--- Page {i+1} ---\n{pg}" for i, pg in enumerate(pages)])
        st.success(f"✅ File Uploaded — {len(pages)} pages detected")
        personas = load_personas()
        sentence_tokenizer = ensure_punkt_tokenizer()
...
"""
                            ...
                            st.markdown(f"<div class='rendered-markdown'>{markdown_output}</div>", unsafe_allow_html=True)

                            st.download_button("💾 Download Summary", markdown_output, file_name="Investor_Summary.md")
...
"""
            ...
            def run_compliance(full_text):
                checklist = fallback_checklist()
                results = []
                for _, row in checklist.iterrows():
                    result = analyze_section(row['Section'], row['Keywords'], full_text)
                    results.append({
                        "✅ Section": row['Section'],
                        "📘 Regulatory Source": row['Regulatory Source'],
                        "📌 Required for": row['Required for'],
        ...
        with tab3:
            ...
            def chunk_by_section(full_text):
                pattern = re.compile(r"(?<=\n)([0-9A-Z]{0,2}[.\)]?\s*)?(?P<title>[A-Z][A-Za-z0-9 /:&,-]{4,60})(?=\n)")
